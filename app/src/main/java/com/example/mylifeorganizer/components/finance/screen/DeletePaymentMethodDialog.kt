...
fun DeletePaymentMethodDialog(
    ...
    AlertDialog(
        ...
        confirmButton = {
            Button(
                onClick = {
                    noteViewModel.updatePaymentIdToNull(paymentId = paymentMethod.paymentId)
                    noteViewModel.deletePaymentMethod(paymentMethod)
                    changeShowDeleteDialog(false)
                },
        ...
        text = {
            Text(
                text = if(isLangEng) {
                    "Are you sure you want to delete the category '${paymentMethod.name}'?"
                } else {
                    "¿Seguro que quieres eliminar la categoría '${paymentMethod.name}'?"
                },
                color = themeColors.text1
            )
        },
